    ...
    <div className="min-h-screen bg-background text-foreground py-6 sm:py-10 px-3 sm:px-6 lg:px-8 pb-20 sm:pb-12">
      <div className="max-w-5xl mx-auto space-y-6 sm:space-y-10">
        ...
        <header className="text-center space-y-3 sm:space-y-4 pt-2 sm:pt-4">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 sm:px-4 sm:py-1.5 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-bold uppercase tracking-wider animate-pulse">
            <Flame className="h-4 w-4 text-amber-500" />
            Live Honor Roll · Real-Time Rankings
          </div>
          <h1 className="text-2xl sm:text-4xl md:text-5xl font-serif font-extrabold tracking-tight text-foreground px-1">
            Donor Leaderboard
          </h1>
          ...
          <div className="flex flex-wrap justify-center items-center gap-2 sm:gap-3 text-xs font-medium text-muted-foreground pt-1">
            <span className="flex items-center gap-1.5 bg-muted/60 px-3 py-1 rounded-full border border-border">
        ...
        {!isLoading && leaderboard.length === 0 && (
          <div className="card-heritage p-8 sm:p-12 text-center space-y-4 max-w-md mx-auto">
            <div className="h-16 w-16 bg-primary/10 text-primary rounded-full flex items-center justify-center mx-auto text-3xl">
              🏆
            </div>
            ...
            <div className="pt-2">
              <Link to="/donate" className="btn-primary w-full inline-flex items-center justify-center gap-2 text-base py-3">
        ...
        {!isLoading && leaderboard.length > 0 && (
          <>
            {/* Top 3 Podium Cards */}
            <section aria-label="Top 3 Donors Podium" className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 items-stretch pt-2 sm:pt-4">
              {/* Silver (Rank 2) */}
              ...
              {/* Gold (Rank 1 - Highlighted & Elevated) */}
              <div className="order-1 md:order-2 md:-translate-y-3 flex">
                {top1 ? (
                  <PodiumCard
                    entry={top1}
                    tierTitle="Gold Plated"
                    badgeIcon={<Crown className="h-7 w-7 text-amber-500 animate-bounce" />}
                    borderColor="border-amber-400 shadow-amber-500/30 ring-2 ring-amber-400/40"
                    bgGradient="bg-gradient-to-b from-amber-100/90 via-amber-500/10 to-card dark:from-amber-950/60 dark:via-amber-900/20 dark:to-card"
                    badgeBg="bg-gradient-to-r from-amber-400 to-yellow-500 text-amber-950 font-black border-amber-300 shadow-md"
                    showAmounts={showAmounts}
                    ribbon="🥇 1st Place · Champion"
                    isChampion
                  />
                ) : (
              ...
              <div className="order-3 md:order-3 flex">
                {top3 ? (
                  <PodiumCard
                    ...
                    bgGradient="bg-gradient-to-b from-amber-100/50 via-card to-background dark:from-amber-950/40 dark:via-card dark:to-background"
            ...
            {restDonors.length > 0 && (
              <section className="space-y-3 sm:space-y-4 pt-4 sm:pt-6">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border pb-2.5 sm:pb-3 px-1">
                  <h2 className="text-lg sm:text-xl font-serif font-bold text-foreground flex items-center gap-2">
                    <Trophy className="h-5 w-5 text-primary" />
                    Honor Roll (Ranks 4+)
                  </h2>
                  <span className="text-xs text-muted-foreground font-medium bg-muted/60 px-2.5 py-1 rounded-full border border-border">
                    {restDonors.length} {restDonors.length === 1 ? "contributor" : "contributors"}
                  </span>
                </div>
                  ...
                  {restDonors.map((donor) => (
                    ...
                    >
                      <div className="flex items-start sm:items-center gap-3 min-w-0 flex-1">
                        {/* Rank Pill */}
                        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-muted border border-border text-foreground font-bold text-xs flex items-center justify-center mt-0.5 sm:mt-0">
                          #{donor.rank}
                        </div>
                        ...
                        {/* Donor Name & Donation Stats - Completely wrapped, never cut off! */}
                        <div className="min-w-0 flex-1">
                          <p className="font-semibold text-foreground text-sm sm:text-base leading-snug break-words">
                            {donor.donor_name}
                          </p>
                      ...
                      <div className="sm:text-right flex-shrink-0 pl-11 sm:pl-0 flex items-center justify-between sm:justify-end gap-2">
                        <span className="sm:hidden text-xs text-muted-foreground font-medium">Contributed:</span>
                        {showAmounts && donor.total_amount !== null ? (
                          <div className="font-serif font-bold text-primary text-base sm:text-lg whitespace-nowrap">
        ...
        <section className="rounded-2xl sm:rounded-3xl bg-gradient-to-r from-primary via-primary/95 to-amber-950 text-primary-foreground p-6 sm:p-10 shadow-2xl relative overflow-hidden text-center space-y-4 sm:space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 relative z-10 pt-1 sm:pt-2">
            <Link
              to="/donate"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-amber-400 hover:bg-amber-300 text-amber-950 font-bold px-6 py-3.5 rounded-full shadow-lg transition-transform hover:scale-105 text-sm sm:text-base"
            >
            ...
            <Link
              to="/kits"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-white font-semibold px-6 py-3.5 rounded-full border border-white/20 backdrop-blur-sm transition-transform hover:scale-105 text-sm sm:text-base"
            >
...
function PodiumCard({
  ...
  return (
    <div
      className={`w-full rounded-2xl border-2 ${borderColor} ${bgGradient} p-5 sm:p-6 text-center shadow-xl transition-transform hover:-translate-y-1 duration-300 flex flex-col justify-between space-y-4`}
    >
      {/* Top Ribbon Badge */}
      <div className="flex justify-center">
        <span
          className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold tracking-wide border shadow-sm ${badgeBg}`}
        >
          {ribbon}
        </span>
      </div>
      ...
      <div className="space-y-3">
        ...
        {/* Donor Name & Category - Full name completely visible with break-words */}
        <div className="space-y-1">
          <h3
            className={`font-serif font-extrabold text-foreground tracking-tight break-words leading-tight ${
              isChampion ? "text-lg sm:text-2xl text-amber-600 dark:text-amber-400" : "text-base sm:text-xl"
            }`}
          >
            {entry.donor_name}
          </h3>
      ...
      <div className="pt-3 border-t border-border/50 space-y-1">
        {showAmounts && entry.total_amount !== null ? (
          <div
            className={`font-serif font-black tracking-tight text-primary break-words ${
              isChampion ? "text-2xl sm:text-3xl text-primary" : "text-xl sm:text-2xl"
            }`}
          >
...
function EmptyPodiumSlot({ rank, label, isChampion = false }: { rank: number; label: string; isChampion?: boolean }) {
  return (
    <div
      className="w-full rounded-2xl border-2 border-dashed border-border bg-muted/20 p-6 text-center flex flex-col items-center justify-center gap-3 min-h-[240px]"
    >
